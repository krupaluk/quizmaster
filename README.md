...
1. Vytvořte datový soubor s konvencí `quizdata_{kategorie}_{id}.js` (např. `quizdata_dejepis_novovek.js`), který začíná definicí: `window.data_dejepis_novovek = { ... }`.
   ...
   Dejepis: {
       ...
       quizzes: {
           dejepisNovovek: {
               ...
               lastUpdated: null, // Automaticky detekováno
               version: "1.0"
           }
       }
   }
   ```
3. Název souboru se automaticky generuje jako `quizdata_{kategorie}_{id}.js`
4. Aplikace téma automaticky zobrazí v příslušné kategorii.

### 📄 Struktura datového souboru kvízu
Příklad `quizdata_filosofie_obdobi_helenismu.js`:
```javascript
